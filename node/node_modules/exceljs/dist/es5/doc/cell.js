...
Cell.prototype = {
  ...
  get commentModel() {
    return this.worksheet.model.comments[this.address];
  },
  get comment() {
//    return "comment"
 //   console.log(this.worksheet.model.comments[this.address])
    if (!this.worksheet.model.comments[this.address]) return null;
    var commentModel = this.worksheet.model.comments[this.address].text;
    if (commentModel.t) {
      return commentModel.t.text;
    } else if (commentModel.r) {
      var commentText = [];
      commentModel.r.forEach(function (r) {
        commentText.push(r.t.text);
      })
      return commentText.join("");
    } else if (commentModel.rPh) {
      return commentModel.rPh.t;
    }
    return this.worksheet.model.comments[this.address].comment;
  },
  set comment(c) {
    var commentModel = {
      t: {text: c}
    }
    this.worksheet.model.comments[this.address] = {author: "exceljs", text: commentModel};
  },
