...
utils.inherits(WorkSheetXform, BaseXform, {
  ...
  prepare: function prepare(model, options) {
    ...
    for (var comment in model.comments) {
      if(model.comments.hasOwnProperty(comment)) {
        model.comments[comment].attributes = {ref: comment};
        options.comments.addOne(model.comments[comment]);
      }
    }
    if (options.comments.getCount() > 0) {
      rId = nextRid(rels);
      rels.push({
        Id: rId,
        Type: RelType.Comment,
  ...
  render: function render(xmlStream, model) {
    ...
    if (model.legacyDrawingId)
      xmlStream.leafNode('legacyDrawing', {'r:id': model.legacyDrawingId})
  ...
  reconcile: function reconcile(model, options) {
    ...
    model.comments = {}
    if (options.comments.commentsList) {
      options.comments.commentsList.forEach(function(comment) {
        model.comments[comment.attributes.ref] = {author: options.comments.authors[comment.attributes.authorId], text: comment.text};
      })
    }
    model.media = [];
    if (model.drawing) {
