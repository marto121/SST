...
var VmlDrawingXform = require('./xform/comment/vmlDrawing-xform');
...
XLSX.prototype = {
  ...
  reconcile: function reconcile(model, options) {
    ...
    model.worksheets.forEach(function (worksheet) {
      ...
      //reconcile comments
      sheetOptions.comments = {};
      worksheet.relationships&&worksheet.relationships.forEach(function(rel){
        var match = rel.Target.match(/\/(comments\d+)[.]xml/)
        if(match) {
         // console.log(model.comments)
          sheetOptions.comments = model.comments[match[1]];
        } else {
        }
      })
  ...
  createInputStream: function createInputStream(options) {
    ...
    stream.on('entry', function (entry) {
      ...
      switch (entryPath) {
        ...
        default:
          promise = self.processWorksheetEntry(entry, model, options) || self.processWorksheetRelsEntry(entry, model) || self.processThemeEntry(entry, model) || self.processMediaEntry(entry, model) || self.processCommentEntry(entry, model) || self.processDrawingEntry(entry, model) || self.processDrawingRelsEntry(entry, model) || self.processIgnoreEntry(entry);
          break;
      }
  ...
  addComments: function addComments(zip, model) {
    var promises = [];
    for (var comment in model.comments) {
      if (model.comments.hasOwnProperty(comment)) {
        promises.push(new PromishLib.Promish(function (resolve) {
          var xform = new CommentsXform();
          var xml = xform.toXml(model.comments[comment]);
          zip.append(xml, { name: 'xl/' + comment + '.xml' });
          xform = new VmlDrawingXform();
          var vmlDrawings = [];
          model.comments[comment].commentList && model.comments[comment].commentList.forEach(function (commentEntry){
            vmlDrawings.push({address: commentEntry.attributes.ref})
          })
          xml = xform.toXml(vmlDrawings);
          zip.append(xml, { name: 'xl/drawings/' + comment.replace('comments', 'vmlDrawing') + '.vml' });
          resolve();
        }));
        }
    }
  ...
  prepareModel: function prepareModel(model, options) {
    ...
    model.worksheets.forEach(function (worksheet) {
      var sheetComments = new CommentsXform();
      sheetComments.prepare();
      worksheetOptions.comments = sheetComments;
      worksheetXform.prepare(worksheet, worksheetOptions);
      if (sheetComments.model.commentList.length>0) 
        model.comments['comments' + worksheet.id] = sheetComments.model;
    });
  ...
  write: function write(stream, options) {
    ...
    .then(function () {
      return _this2.addComments(zip, model)
    }).then(function () {
