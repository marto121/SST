...
SELECT Asset_History.Asset_ID, LastDate.CurrMonth, Asset_History.Status_Code, Asset_History.Book_Value, 0 AS OPEX, 0 AS CAPEX,  case when  Expected_Exit is null  then '2000-1-1' else asset_History.expected_Exit end  AS Expected_Exit_, Asset_History.m_ID
...
SELECT NPE_List.NPE_Code, NPE_List.NPE_Name, Nom_Scenarios.Scenario_Name, Nom_Currencies.Currency_Code, NPE_List.NPE_Amount_CCY, NPE_List.NPE_Amount_EUR, NPE_List.LLP_Amount_CCY, NPE_List.LLP_Amount_EUR, NPE_List.NPE_Owned, NPE_History.Rep_Date, Nom_NPE_Status.NPE_Status_Code, Max(Currencies_1.Currency_Code) AS Max��Currency_Code, Sum(Asset_Repossession.NPE_Amount_CCY) AS Sum��NPE_Amount_CCY, Sum(Asset_Repossession.NPE_Amount_EUR) AS Sum��NPE_Amount_EUR, Sum(Asset_Repossession.LLP_Amount_CCY) AS Sum��LLP_Amount_CCY, Sum(Asset_Repossession.LLP_Amount_EUR) AS Sum��LLP_Amount_EUR
...
UPDATE ((Asset_History AS new_tab INNER JOIN Assets_List AS old_parent ON old_parent.id=new_tab.Asset_ID) INNER JOIN Assets_List AS new_parent ON new_parent.Asset_Code=old_parent.Asset_Code) INNER JOIN Asset_History AS dw_tab ON (dw_tab.Rep_Date=new_tab.Rep_Date) AND (dw_tab.Asset_ID=new_parent.id) SET dw_tab.Status_Code =  case when new_tab.Status_Code is null then dw_tab.Status_Code else new_tab.Status_Code end , dw_tab.Inflows =  case when new_tab.Inflows is null then dw_tab.Inflows else new_tab.Inflows end , dw_tab.CAPEX =  case when new_tab.CAPEX is null then dw_tab.CAPEX else new_tab.CAPEX end , dw_tab.Depreciation =  case when new_tab.Depreciation is null then dw_tab.Depreciation else new_tab.Depreciation end , dw_tab.Sales =  case when new_tab.Sales is null then dw_tab.Sales else new_tab.Sales end , dw_tab.Imp_WB =  case when new_tab.Imp_WB is null then dw_tab.Imp_WB else new_tab.Imp_WB end , dw_tab.Book_Value =  case when new_tab.Book_Value is null then dw_tab.Book_Value else new_tab.Book_Value end , dw_tab.Costs =  case when new_tab.Costs is null then dw_tab.Costs else new_tab.Costs end , dw_tab.Income =  case when new_tab.Income is null then dw_tab.Income else new_tab.Income end , dw_tab.Expected_Exit =  case when new_tab.Expected_Exit is null then dw_tab.Expected_Exit else new_tab.Expected_Exit end 
WHERE new_tab.m_id = p_m_ID and dw_tab.m_id=-1
 and new_parent.m_id=-1;
...
SELECT NPE_List.NPE_Code, NPE_List.NPE_Name, Max(Nom_NPE_Status.NPE_Status_Text) AS Status, Max( case when npe_history.npe_scenario_id=11 then npe_history.npe_rep_date else Null end ) AS FC_Rep_Date, NPE_History.NPE_Currency, Sum( case when npe_history.npe_scenario_id=11 then npe_history.npe_amount_ccy else 0 end ) AS FC_NPE_CCY, Sum( case when npe_history.npe_scenario_id=11 then npe_history.npe_amount_eur else 0 end ) AS FC_NPE_EUR, NPE_List.NPE_Scenario_ID, Sum( case when npe_history.npe_scenario_id=2 then npe_history.npe_amount_eur else 0 end ) AS MYP_Original, Sum( case when npe_history.npe_scenario_id=3 then npe_history.npe_amount_eur else 0 end ) AS MYP_Update, Sum( case when npe_history.npe_scenario_id=6 then npe_history.npe_amount_eur else 0 end ) AS Actual
