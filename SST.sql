...
Financing_Reference_Rate varchar(10),
...
ALTER TABLE Users ADD constraint Users_PrimaryKey primary key (ID);
...
$$;
-- 29.12.2017 ใ. 9:52:24
CREATE VIEW NPE_per_scenario AS
TRANSFORM Sum(NPE_History.NPE_Amount_EUR) AS SumอเNPE_Amount_EUR
...
WHERE tab.m_id in (p_m_ID,-1)
GROUP BY parent.Asset_Code, parent.Asset_Name, Nom_Financing_Type1.Financing_Type, tab.Financing_Start_Date
HAVING max(tab.m_id)>-1;

-- 18.2.2018 ใ. 21:21:01
CREATE VIEW sel_Asset_History AS
SELECT parent.Asset_Code, parent.Asset_Name, tab.Rep_Date, max(iif(tab.m_id=-1, Nom_Asset_Status0.Asset_Status_Code,null)) AS old_Status_Code, max(iif(tab.m_id<>-1, Nom_Asset_Status0.Asset_Status_Code,null)) AS new_Status_Code, max(iif(tab.m_id=-1, tab.Inflows,null)) AS old_Inflows, max(iif(tab.m_id<>-1, tab.Inflows,null)) AS new_Inflows, max(iif(tab.m_id=-1, tab.CAPEX,null)) AS old_CAPEX, max(iif(tab.m_id<>-1, tab.CAPEX,null)) AS new_CAPEX, max(iif(tab.m_id=-1, tab.Depreciation,null)) AS old_Depreciation, max(iif(tab.m_id<>-1, tab.Depreciation,null)) AS new_Depreciation, max(iif(tab.m_id=-1, tab.Sales,null)) AS old_Sales, max(iif(tab.m_id<>-1, tab.Sales,null)) AS new_Sales, max(iif(tab.m_id=-1, tab.Imp_WB,null)) AS old_Imp_WB, max(iif(tab.m_id<>-1, tab.Imp_WB,null)) AS new_Imp_WB, max(iif(tab.m_id=-1, tab.Book_Value,null)) AS old_Book_Value, max(iif(tab.m_id<>-1, tab.Book_Value,null)) AS new_Book_Value, max(iif(tab.m_id=-1, tab.Costs,null)) AS old_Costs, max(iif(tab.m_id<>-1, tab.Costs,null)) AS new_Costs, max(iif(tab.m_id=-1, tab.Income,null)) AS old_Income, max(iif(tab.m_id<>-1, tab.Income,null)) AS new_Income, max(iif(tab.m_id=-1, tab.Expected_Exit,null)) AS old_Expected_Exit, max(iif(tab.m_id<>-1, tab.Expected_Exit,null)) AS new_Expected_Exit, iif(min(tab.m_ID)>-1,"New","") AS Record_Status
...
WHERE tab.m_id in (p_m_ID,-1)
GROUP BY parent.Asset_Code, parent.Asset_Name, tab.Sale_Approval_Date
HAVING max(tab.m_id)>-1;
...
WHERE tab.m_id in (p_m_ID,-1)
GROUP BY parent.NPE_Code, parent.NPE_Name, tab.Rep_Date, Nom_Scenarios1.Scenario_Name
HAVING max(tab.m_id)>-1;
...
FROM Nom_Currencies RIGHT JOIN (((Nom_Countries RIGHT JOIN NPE_List ON Nom_Countries.ID = NPE_List.NPE_Country_ID) LEFT JOIN vwCounterparts AS Lender ON NPE_List.NPE_Lender_ID = Lender.ID) LEFT JOIN vwCounterparts AS Borrower ON NPE_List.NPE_Borrower_ID = Borrower.ID) ON Nom_Currencies.ID = NPE_List.NPE_Currency_ID
...
SELECT Users.EMail, Nom_Countries.Country_Code, Users.Role
